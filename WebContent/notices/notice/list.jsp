...
<body>
	...
	<form action="searchNotice.do" method="post">
		...
		<a class="textnone" href="insertNotice.do">[공지사항 등록]</a>
	...
	<p class="notice" dir="ltr">
		<span style="font-size: 14px"> <strong><%=notList.get(i).getNotTit()%></strong>
			<span><%=notList.get(i).getNotDt()%></span> <br> <%=notList.get(i).getNotCon().replaceAll("%", "<br>")%>
			<%
				if (admin != null && member == null) {
			%> <br> <span><a class="textnone"
				href="updateNotice.do?notId=<%=notList.get(i).getNotId()%>">[공지사항
					수정]</a></span> <br> <span><a class="textnone"
				href="deleteNotice.do?notId=<%=notList.get(i).getNotId()%>">[공지사항
					삭제]</a></span> <br> <%
 	}
 %>
			<hr> <%
